...
import { getAllUsers, getOrCreateChat, sendMessage } from "@/lib/firebase/firestore";
...
export default function MessagesPage({ params: { lang } }: { params: { lang: string } }) {
    ...
    // Load all users to allow messaging anyone
    useEffect(() => {
        if (user) {
            getAllUsers().then(users => {
                setContacts(users.filter((u: any) => u.id !== user.uid));
            });
        }
    }, [user]);
            ...
            <div className={`w-1/3 border-${isArabic ? 'l' : 'r'} border-gray-200 bg-gray-50 flex flex-col`}>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-2">
                    {contacts.map(contact => (
                        <div 
                            key={contact.id}
                            onClick={() => handleSelectTeacher(contact.id)}
                        ...
                        >
                            ...
                            <div className="truncate">
                                <p className="font-bold text-gray-800 truncate">{contact.email?.split('@')[0] || "User"}</p>
                    ...
                    {contacts.length === 0 && (
                        <p className="text-gray-500 text-sm text-center mt-4">
                            {isArabic ? "لا يوجد مستخدمين متاحين." : "No users available."}
                        </p>
                    )}
                </div>
            </div>
            ...
            <div className="flex-1 flex flex-col bg-white relative">
                {selectedChatId ? (
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-gray-400">
                        ...
                        <p>{isArabic ? "اختر جهة اتصال لبدء المحادثة" : "Select a contact to start chatting"}</p>
                    </div>
                )}
            </div>
        </div>
